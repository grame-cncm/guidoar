...
Now and for each application type, we'll examine the bounded evaluation equivalence. Note that we'll make use the following equivalence:
\hspace{1cm} $\evalexpr[\nobounds]{e} \rightarrow \evalsimple{e}$

\evaltable
\evaltitle {Score application equivalence}
 from the above: \: $\evalexpr{score \applyop w}$ 	& & $\apply{\evalexpr[\nobounds]{score}}{\evalexpr{w}}$ \\
									& & $\applysimple{score}{\equivalence{\evalsimpleshort{w}}}$ \\
									& & $\noredex(score, \equivalence{w})$
\evalspace
from the evaluation \evalsym\ def: \: $\equivalence{\evalsimpleshort{score \applyop w}}$  & & $\equivalence{\noredex(score,w)}$ \\
									& &  $\noredex(score, \equivalence{w})$
\evalspace
...
\evaltitle {Sequence application equivalence}
 $\evalexpr{(u \seq v) \applyop w}$ 	& \ra & $\apply{\evalexpr[\nobounds]{u \seq v}}{\evalexpr{w}}$ \\
						& \ra & $\apply{seq(u, v)}{w'}$ where $w' = \evalexpr{w}$ \\
						& \ra & $seq(\apply{u}{\head(w',\dur(u))}, \apply[\bounds(l:=l-l_{uw})]{v}{\tail(w',\dur(u))}))$ \\
				 		& &		\: where $l_{uw}= \length(\apply{u}{\head(w',\dur(u))})$ \\
%						& \ra & $seq(\apply{u}{\head(w',\dur(u))}, \apply[\bounds(l:=l-l_{uw})]{v}{\tail(w',\dur(u))}))$ \\
						& \ra & $seq(\equivalence{h_{uw}}, \equivl{t_{uw}}{l-l_{uw}})$ \\
				 		& &		\: where $h_{uw}=\applysimple{u}{\head(w',\dur(u))}$ and $t_{uw} = \applysimple{v}{\tail(w',\dur(u))} $
%									& & $\applysimple{\evalsimpleshort{u \seq v}}{\equivalence{\evalsimpleshort{w}}}$ \\
%									& & $\applysimple{seq(u, v)}{\equivalence{\evalsimpleshort{w}}}$ \\
%									& & $seq(\applysimple{u}{\head(w', \dur(u))}, \applysimple{v}{\tail(w', \dur(u))})$ 
%									with $w'= \equivalence{\evalsimpleshort{w}}$
%\evalspace
\evaltablend
from the evaluation \evalsym\ def:
\evaltable[]
 $\equivalence{\evalsimpleshort{(u \seq v) \applyop w}}$  
 	& \ra & $\equivalence{\applysimple{seq(u, v)}{w''}}$ with $w''=\evalsimpleshort{w}$\\
	& \ra &  $\equivalence{seq(\applysimple{u}{\head(w'',\dur(u))}, \applysimple{v}{\tail(w'',\dur(u))})}$ \\
	& \ra &  $\head(seq(\top(\applysimple{u}{\head(w'',\dur(u))}, \bounds(v)),\top(\applysimple{v}{\tail(w'',\dur(u))},\bounds(v))),\bounds(l))$ \\
	& \ra &  $\head(seq(\top(h_{uw}', \bounds(v)), \top(t_{uw}', \bounds(v))),\bounds(l))$ \\
	& &  \: where $h_{uw}' = \applysimple{u}{(\head(w'',\dur(u))}$ \\
	& &  \: and   $t_{uw}' = \applysimple{v}{\tail(w'',\dur(u))}$ \\
...
\begin{center} \begin{tabular*}{0.9\linewidth}{rclcl}
	$h_{uw}'$ 	& $=$ & $\applysimple{u}{(\head(w'',\dur(u))}$ & $=$ & $\applysimple{u}{(\head(\evalsimpleshort{w},\dur(u))}$ \\
	$h_{uw}$ 	& $=$ & $\applysimple{u}{\head(w',\dur(u))}$ 	& $=$ & $\applysimple{u}{\head(\evalexpr{w},\dur(u))}$
\evalspace
	$t_{uw}'$ 	& $=$ & $\applysimple{v}{\tail(w'',\dur(u))}$ 	& $=$ & $\applysimple{v}{\tail(\evalsimpleshort{w},\dur(u))}$ \\
	$t_{uw}$ 	& $=$ & $\applysimple{v}{\tail(w',\dur(u))}$ 	& $=$ & $\applysimple{v}{\tail(\evalexpr{w},\dur(u))}$
\evalspace
	$l_{uw}'$ 	& $=$ & $\length(h_{uw}')$	& $=$ & $\length(\applysimple{u}{(\head(\evalsimpleshort{w},\dur(u))})$	\\
	$l_{uw}$ 	& $=$ & $\length(\apply{u}{\head(w',\dur(u))})$	& $=$ & $\length(\apply{u}{\head(\evalexpr{w},\dur(u))})$	
\evaltablend


thus $\evalexpr{score \applyop w}$ 	\ra $\: \equivalence{\evalsimpleshort{score \applyop w}}$


%%%%%% Mis application evaluation proof %%%%%%
%\evaltable
%\evaltitle {Parallel application equivalence}
% from the above: \: $\evalexpr{(u \mix v) \applyop w}$ 	& & $\apply{\evalexpr[\nobounds]{u \mix v}}{\evalexpr{w}}$ \\
%									& & $\applysimple{\evalsimpleshort{(u \mix v)}}{\equivalence{\evalsimpleshort{w}}}$ \\
%\evalspace
%from the evaluation \evalsym\ def: \: $\equivalence{\evalsimpleshort{(u \mix v) \applyop w}}$  & & $\equivalence{(u \mix v) \applyop w)}$ \\
%									& &  $\noredex(score, \equivalence{w})$
%\evalspace
%thus $\evalexpr{score \applyop w}$ 	& & $\equivalence{\evalsimpleshort{score \applyop w}}$
%\evaltablend
