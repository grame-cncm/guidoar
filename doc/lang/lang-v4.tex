...
\evalspace
 $\apply{\closenv{\bounds'}}{w}$  & $\rightarrow$ & $\evalexpr{f}[\var{e} := w]$ \\
...
\evalspace
from the simple evaluation def: \: $\equivalence{\evalsimpleshort{score \applyop w}}$  & & $\equivalence{\noredex(score,w)}$ \\
									& &  $\noredex(score, \equivalence{w})$
\evalspace
...
\evaltitle {Sequence application equivalence}
 from the above: \: $\evalexpr{(u \seq v) \applyop w}$ 	& & $\apply{\evalexpr[\nobounds]{u \seq v}}{\evalexpr{w}}$ \\
									& & $\applysimple{\evalsimpleshort{(u \seq v)}}{\equivalence{\evalsimpleshort{w}}}$ \\
\evalspace
from the simple evaluation def: \: $\equivalence{\evalsimpleshort{(u \seq v) \applyop w}}$  & & $\equivalence{(u \seq v) \applyop w)}$ \\
									& &  $\noredex(score, \equivalence{w})$
\evalspace
thus $\evalexpr{score \applyop w}$ 	& & $\equivalence{\evalsimpleshort{score \applyop w}}$
\evaltablend
