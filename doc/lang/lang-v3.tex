...
\newcommand{\noredex}{@^o}
\newcommand{\f}[4]{$#1 \times #2 \rightarrow #3 \times #4$}
\newcommand{\fv}[3]{$#1 \times #2 \rightarrow #3$}
\newcommand{\fb}{\mathbb{N} \times \mathbb{N} \times \mathbb{Q}}
\newcommand{\evalsym}{\epsilon}
\newcommand{\eval}[1]{\evalsym[\bounds](#1,\rho)}
\newcommand{\evala}[1]{\evalsym(#1,\rho)}
\newcommand{\evalb}[2]{\evalsym[#2](#1,\rho)}
\newcommand{\envsym}{\rho}
\newcommand{\emptyenv}{\theta}
\newcommand{\closenv}[5][\var{e}]{\closure[#3,#4,#5,\lambda[#1,#2],\envsym]}

%%%%%%%%%%%%%%% definitions for functions symbols
\newcommand{\apply}		{{\mathcal A}[\bounds]}
...
\begin{tabular}{rc@{ : }l}
 ...
 $\closenv{f}{l}{v}{d}$ 		& & a closure associating a length $l$, a voices number $v$,  a duration $d$,  a $\lambda$-abstraction and an environment $\envsym$ \\
...
\begin{center}
\begin{tabular}{rc@{ : }l}
$\eval{e}$ & & evaluates an expression $e$ from \f{\expr}{\env}{\val}{\elength} using an \emph{environment} $\envsym$ and a \emph{bound} $\bounds$ \\
$A(u,v)$  & & from \f{\val}{\val}{\val}{\elength}, apply the function $u$ to the argument $v$. \\
\end{tabular}
\end{center}
...
A \emph{bound} $\bounds$ is a triplet $[l, v, d] \in \fb$ used to bound the evaluation process in length using $l$, in voices number using $v$, or in duration using $d$. 
The notation $\bounds(l)$, $\bounds(v)$ and $\bounds(d)$ respectively refers to the value of the length $l$, the voice number $v$ and the duration $d$  of $\bounds$. $\bounds[l := x]$ represents a bound where the length $l$ is set to $x$; $\bounds[l := x, d:=y]$ represents a bound where the length $l$ is set to $x$ and the duration $d$ to $y$, etc...

The semantic of a \emph{bounded} evaluation $\eval{e}$ is similar to $\head[ \top[ \evala{e}, \bounds(v)], \bounds(l)]$. The duration $d$ of a bound $\bounds$ is used for temporal applications.
...
The evaluation function $\eval{l}{e}$ : \f{\expr}{\env}{\val}{[\fb]} is defined as follow:
...
\begin{tabular}{rc@{ $\rightarrow$ }l}
 \hline
  $\eval{score}$	& & $\left\{ \begin{array}{ll}
							...
							\left[ score, \bounds(l := l_{score}) \right] & \mathrm{when} \  l_{score} \geq \bounds(l) \\
 										& \mathrm{with} \   l_{score} = \length (score) \\
	 							\end{array} \right.$ \\
 $\eval{\var{e}}$ 				& & $[ \envsym(\var{e}), \bounds]$ \\
% $\eval{\lambda[\var{e},f]}$ 	& & $[ \closenv{f}{l}, \bounds]$ \\
 $\eval{\lambda[\var{e},f]}$ 	& & $\closenv{f}{l}{v}{d}$ with $l=\length(\evalsym(f,\envsym[\var{e}= \evalsym(e, \emptyenv)))$ and $d=$ \\
 $\eval{@[e,f]}$ 				& & $[ \apply{l} [\eval{inf}{e}, \eval{inf}{f}), l]$ where $inf$ is an infinite length\\
 $\eval{\seq [e,f]}$	& & $\left\{ \begin{array}{ll}
							\eval{l}{e} & \mathrm{when} \  l_e \geq l \\
							\left[ seq[\eval{l}{e}, \eval{l-l_e}{f}), l_e + l_f \right] & \mathrm{when} \  l_e < l \\
 										& \mathrm{with} \   l_e = \eval{l}{e} \mathrm{and} \   l_f = \eval{l-l_e}{f} \\
	 							\end{array} \right.$ \\
% $\eval{/ [e,f]}$ 			& & $mix[\eval{e}, \eval{f}]$ \\
\end{tabular}
\begin{tabular}{rc@{ $\rightarrow$ }ll}
 $\eval{l}{-| [e,f]}$		& & $\eval{l_f}{e}$ & with $l_f = \eval{l}{f})$ \\
 $\eval{l}{|- [e,f]}$		   & & $[ \tail [\eval{l+l_f}{e},l_f], max(0,l_e - l_f)]$ & with $l_f = \eval{l}{f})$ 
 										and $l_e = \eval{l+l_f}{e})$ \\
...
% $\eval{transp[e,f]}$ 		& & $\transp [\eval{e},i)$ with $i = \pitch(\eval{f}) - \pitch(\eval{e}) $ \\
 $\eval{l}{transp[e,f]}$ 		& & $[\transp [\eval{l}{e},i], l_e]$ & with $i = \pitch(\eval{l}{f}) - \pitch(\eval{l}{e}) $ 
 										and $l_e = \eval{l}{e} $\\
 $\eval{l}{stretch[e,f]}$ 	& & $[\stretch [\eval{l}{e},r], l_e \times r]$ & with $r = \dur(\eval{l}{f} / \dur(\eval{l}{e}))$ 
										and $l_e = \eval{l}{e} $\\
...
%----------------------------------------------------------------
\subsubsection{The application function $\apply{l}()$}
The application function $\apply{l}(u,v)$ : \fv{\val}{\val}{\val} applies $u$ to $v$ on a length $l$ as follow:

\begin{center}
\begin{tabular}{rc@{ $\rightarrow$ }l}
 \hline
 $\apply{l}(score,w)$					& & $\noredex[score,w]$ \\
 $\apply{l}(\seq [u,v],w)$			& & $\left\{ \begin{array}{ll}
							 \apply{l}(u,w) & \mathrm{when} \  l_{u} \geq l \\
							 seq[\apply{l_u}(u,\head(w,\dur(u))), \apply{l-l_u}(v,\tail(w,\dur(u)))] & \mathrm{when} \  l_u < l \\
 										& \mathrm{with} \   l_u = \length (u) \\
	 							\end{array} \right.$ \\
% $\apply{l}(\seq [u,v],w)$			& & $seq[\apply(u,\head(w,\dur(u))), \apply(v,\tail(w,\dur(u)))]$ \\
 $\apply{l}(/ [u,v],w)$ 				& & $mix[\apply(u,w),\apply(v,w)]$ \\
 $\apply{l}(\noredex [u,v],w)$ 		& & $\noredex[\noredex[u,v],w]$ \\
 $\apply{l}(\closenv{f}{l}{d},w)$ 	& & $\eval{l}(f,\envsym[\var{e} := w])$ \\
 \hline
\end{tabular}
\end{center}
...
The function $\transp(u,n)$ :  \fv{\val}{\mathbb{Z}}{\val} operates a $n$ chromatic pitch transposition:
...
\begin{tabular*}{12cm}{rc@{ $\rightarrow$ }l}
 ...
 $\transp(\noredex [u,v],n)$ 	& & $\noredex[\transp(u,n),v]$ \\
 $\transp(\closenv{f}{l}{v}{d},n)$ 	& & $\closenv{\transp(f,n)}{l}{v}{d}$ \\
 \hline
\end{tabular*}
\end{center}
...
The function $\stretch(u,r)$ :  \fv{\val}{\mathbb{R}}{\val} stretches $u$ using a coefficient $r$:
...
\begin{tabular*}{12cm}{rc@{ $\rightarrow$ }l}
 ...
 $\stretch(\closenv{f}{l}{v}{d},r)$ 	& & $\closenv{\stretch(f,r)}{l}{v}{d}$ \\
 \hline
\end{tabular*}
\end{center}
...
The function $\head(u,l)$ :  \fv{\val}{\mathbb{N}}{\val} computes the head of $u$ over the length $l$:
...
\begin{tabular*}{12cm}{rc@{ $\rightarrow$ }l}
 ...
 $\head(\noredex [u,v],l')$ 	& & $\noredex[\head(u,l'),v]$ \\
 $\head(\closenv{f}{l}{v}{d},l')$ 	& &  $\closenv{f}{min(l,l')}{v}{d}$ \\
 \hline
\end{tabular*}
\end{center}
...
The function $\tail(u,l)$ :  \fv{\val}{\mathbb{N}}{\val} computes the tail of $u$ after the length $l$:
...
\begin{tabular*}{12cm}{rc@{ $\rightarrow$ }l}
 ...
 $\tail(\noredex [u,v],l')$ 	& & $\noredex[\tail(u,l'),v]$ \\
 $\tail(\closenv{f}{l}{v}{d},l')$ 	& &  $\left\{ \begin{array}{ll}
 								\closenv{f}{l-l'}{v}{d} & \mathrm{when} \  l' < l \\
...
The function $\top(u,n)$ :  \fv{\val}{\mathbb{N}}{\val} computes the top of $u$ over the $n$ first voices:
...
\begin{tabular*}{12cm}{rc@{ $\rightarrow$ }l}
 ...
 $\top(/ [u,v],n)$ 			& & $\left\{ \begin{array}{ll}
								...
								\top(u,n) & \mathrm{when} \  n \leq\voices(u) \\
	 							\end{array} \right.$ \\
 $\top(\noredex [u,v],n)$ 	& & $\noredex[\top(u,n),v]$ \\
 $\top(\closenv{f}{l}{v}{d},n)$ 	& &  $\closenv{\top(f)}{l}{v}{d}$ \\
 \hline
\end{tabular*}
\end{center}
...
The function $\bottom(u,n)$ :  \fv{\val}{\mathbb{N}}{\val} computes the bottom of $u$ after the $n$ first voices:
...
\begin{tabular*}{12cm}{rc@{ $\rightarrow$ }l}
 ...
 $\bottom(/ [u,v],n)$ 			& & $\left\{ \begin{array}{ll}
								...
								\bottom(v,n - \voices(u)) & \mathrm{when} \  n \geq\voices(u) \\
	 							\end{array} \right.$ \\
 $\bottom(\noredex [u,v],n)$ 	& & $\noredex[\bottom(u,n),v]$ \\
 $\bottom(\closenv{f}{v}{l}{d},n)$ 	& &  $\closenv{\bottom(f)}{l}{v}{d}$ \\
 \hline
\end{tabular*}
\end{center}
