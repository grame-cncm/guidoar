...
\newcommand{\bounddomain}{\mathbb{N} \times \mathbb{N} \times \mathbb{Q}}
...
A \emph{bound} $\bounds$ is a triplet $[l, v, d] \in \bounddomain$ used to bound the evaluation process in length using $l$, in voices number using $v$, or in duration using $d$. 
...
\evaltitle {Abstraction, application and environments evaluation} 
 ...
 \evalspace
 $\eval{\var{e}}$ 				& $\rightarrow$ & $[ \envsym(\var{e}), \bounds]$
\evalspace
 $\eval{\lambda[\var{e},f]}$ 	& $\rightarrow$& $[ \closenv{f}{l}{v}{d}, \bounds]$ \\
 						& & with $l=\length(\evalmod[\nobounds]{f}{\ \envsym[\var{e}= \evalmod[\nobounds]{e}{\emptyenv}}$ \\
						& & and $v=\voices(\evalmod[\nobounds]{f}{\ \envsym[\var{e}= \evalmod[\nobounds]{e}{\emptyenv}}$ \\
						& & and $d=\dur(\evalmod[\nobounds]{f}{\ \envsym[\var{e}= \evalmod[\nobounds]{e}{\emptyenv}}$ 
\evaltablend
...
$\eval{stretch[e,f]}$ 		& $\rightarrow$& $[\stretch [\eval{e},r], \bounds[d:=d_e \times r]]$  \\
					& & \ with $r = \dur(\eval{f} / \dur(\eval{e}))$ and $d_e \leftarrow \eval{e} $
\evaltablend
...
\evaltitle {Applications in sequence or in parallel} 
 $\apply(\seq [u,v],w)$			& & $\left\{ \begin{array}{ll}
							 \apply(u,w) & \mathrm{when} \  d_{u} \geq \bounds(d) \\
							 \left[ seq(\apply[{\bounds[d:=d_u]}](u,w_h), \apply[{\bounds[d:=d-d_u]}](v,w_t)),\bounds \right] & \mathrm{when} \  d_u < \bounds(d) \\
 							\  \mathrm{with} \   d_u = \length (u), \  w_h=\head(w,d_u) \ \mathrm{and} \   w_t=\tail(w,d_u) & \\
	 							\end{array} \right.$ \\
\evalspace
...
\evaltitle {A closure application} 
 $\apply(\closenv{f}{l}{v}{d},w)$ 	& & $\evalmod{f}{\envsym[\var{e} := w]}$
\evaltablend
