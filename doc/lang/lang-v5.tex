...
\newcommand{\evalss}[1]				{\evalsym\left[#1\right]}
...
\newcommand{\equivv}[2]				{\head( \top (#1, \bounds(#2)), \bounds(l))}
...
\newcommand{\mixOp}[2]						{\ensuremath{par\left(#1, #2\right)}}
...
\newcommand{\sevalLambda}[4][\envsym]		{\ensuremath{\closure\left(#4,\ [\lambda \var{#2}.#3],\ #1\right)}}
...
\newcommand{\sapplySeq}[3]					{\ensuremath{seq\left(\applysimple{#1}{\head\left(#3,\dur\left(#1\right)\right)}, \applysimple{#2}{\tail\left(#3,\dur\left(#1\right)\right)}\right)}}
...
\newcommand{\durScore}						{\ensuremath{max\left(\dur\left(v_1\right),..., \dur\left(v_n\right)\right)}}
\newcommand{\durSeq}[2]						{\ensuremath{\dur\left(#1\right) + \dur\left(#2\right)}}
\newcommand{\durMix}[2]						{\ensuremath{max\left(\dur\left(#1\right),\dur\left(#2\right)\right)}}
\newcommand{\durApply}[2]						{\ensuremath{\dur\left(#2\right)}}
...
\newcommand{\transpClosure}[5][\envsym]		{\ensuremath{\closure\left(#4,\ [\lambda \var{#2}.\transp\left(#3,#5\right)],\ #1\right)}}
...
\newcommand{\stretchClosure}[5][\envsym]	{\ensuremath{\closure\left(#4\left(d:=d*#5\right),\ [\lambda \var{#2}. \stretch\left(#3,#5\right)],\ #1\right)}}
...
\newcommand{\headClosure}[5][\envsym]		{\ensuremath{\closure \left(#4\left(l:=min\left(l,#5 \right)\right),\ \left[\lambda \var{#2}. \head\left(#3,#5 \right) \right],\ #1\right)}}

\newcommand{\tailScore}[2]					{\ensuremath{#1_{#2'}'}}
\newcommand{\tailSeq}[3]						{\ensuremath{seq\left(\tail\left(#1, #3\right), \tail\left(#2, \ #3 - \length\left(#1\right)\right)\right)}}
...
\newcommand{\tailClosure}[5][\envsym]		{\ensuremath{\closure\left(#4\left(l:=l-#5\right),\ [\lambda \var{#2}. \tail\left(#3,#5\right)],\ #1\right)}}

\newcommand{\topScore}[2]						{\ensuremath{#1_{#2'}'}}
\newcommand{\topSeq}[3]						{\ensuremath{seq\left(\top\left(#1, #3\right), \top\left(#2, \ #3\right)\right)}}
\newcommand{\topMix}[3]						{\ensuremath{mix\left(\top\left(#1, #3\right), \top\left(#2, #3-\voices\left(#1\right)\right)\right)}}
\newcommand{\topApply}[3]						{\ensuremath{\noredex\left(#1, \top\left(#2,#3\right)\right)}}
\newcommand{\topClosure}[5][\envsym]			{\ensuremath{\closure\left(#4\left(v:=min\left(v,#5\right)\right),\ [\lambda \var{#2}. \top\left(#3,#5\right)],\ #1\right)}}
...
\newcommand{\bottomClosure}[5][\envsym]		{\ensuremath{\closure\left(#4\left(v:=v-#5\right),\ [\lambda \var{#2}. \bottom\left(#3,#5\right)],\ #1\right)}}
...
\evaltable[]
 ...
 $\evalsimple{\lambdaexpr{e}{f}}$ 	& \ra & \sevalLambda{e}{f}{\bounds'} \\
...
\evaltable
 ...
 $\applysimple{\closenv{\bounds}}{w}$ 	& & \sapplyClosure{e}{f}{w} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\length(\noredex (u,v))$ 		& & $\length(v)$ \\
 $\length(\closenv{\bounds})$ 	& & \lengthClosure{e}{f}{\bounds} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\dur(\closenv{\bounds})$ 	& & \durClosure{e}{f}{\bounds} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\transp(\closenv{\bounds},n)$ 	& & \transpClosure{u}{v}{\bounds}{n} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\stretch(\closenv{\bounds},r)$ 	& & \stretchClosure{u}{v}{\bounds}{r} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\head(\closenv{\bounds},l')$ 	& & \headClosure{e}{f}{\bounds}{l'} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\tail(\closenv{\bounds},l')$ 	& & \tailClosure{e}{f}{\bounds}{l'} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\top(\closenv{\bounds},n)$ 	& & \topClosure{e}{f}{\bounds}{n} \\
 \hline
\evaltablend
...
\evaltable
 ...
 $\bottom(\closenv{\bounds},n)$ 	& & \bottomClosure{e}{f}{\bounds}{n} \\
 \hline
\evaltablend
...
and when $\bounds(l) < \length(e')$ & &  $\evalexpr{\opexpr{\seq}} \ra \seqOp{\headOp{e'}{\bounds(l)}}{\emptyScore}$ \\
	&   &  and $\headSeq{e'}{f'}{\bounds(l)} \ra \seqOp{\headOp{e'}{\bounds(l)}}{\emptyScore}$
\evalspace
...
\evaltable
 $\evalexpr{\lambdaexpr{e}{f}}$ 	&  & $\result{\closenv{\bounds'(l:=min(l,\bounds(l)), v:=min(v,\bounds(v)), d:=min(d,\bounds(d)))}} 
 				{\bounds'}$ \\
\evaltablend

By definition of $\evalsimple{\lambdaexpr{e}{f}}$  (section \ref{evalsimple}):
\evaltable[]
 $\evalsimple{\lambdaexpr{e}{f}}$ 	& \ra & $\closenv{\bounds'}$ \\
 								& & \ with $\bounds'(l:=\length(VAL(f)), v:=\voices(VAL(f)), d=\dur(VAL(f)))$ \\
			 			& & \ and $VAL(f) = \evalss{f},\envsym[\var{e}= \evalss{e}, \emptyenv]$ \\
and $\head( \top (\evalss{\lambdaexpr{e}{f}}, \bounds(v)), \bounds(l))$ &  \ra & $\head( \top (\closenv{\bounds'}, \bounds(v)), \bounds(l))$ \\
...
\evaltitle {Score application equivalence}
 from the above: \: $\evalexpr{score \applyop w}$ 	& & $\apply{\evalss{score}}{\evalss{w}}$ \\
									& & $\applysimple{score}{\equivalence{\evalss{w}}}$ \\
									& & $\noredex(score, \equivalence{\evalss{w}})$
\evalspace
from \evalsym\ definition: \: $\equivalence{\evalss{score \applyop w}}$  & & $\equivalence{\noredex(score, \evalss{w}}$ \\
									& &  $\noredex(score, \equivalence{\evalss{w}})$
\evalspace
...
\evaltitle {Sequence application equivalence}
 $\evalexpr{(u \seq v) \applyop w}$ 
	...
	& \ra & $\apply{seq(u, v)}{\evalss{w}}$ \\
	& \ra & $seq(\apply{u}{\head(\evalss{w},\dur(u))}, \apply[\bounds(l:=l-l_{uw})]{v}{\tail(\evalss{w},\dur(u))}))$ \\
	& &		\: where $l_{uw}= \length(\apply{u}{\head(\evalss{w},\dur(u))})$ \\
	& \ra & $seq(\equivalence{h_{uw}}, \equivl{t_{uw}}{l-l_{uw}})$ \\
	& &		\: where $h_{uw}=\applysimple{u}{\head(\evalss{w},\dur(u))}$ and $t_{uw} = \applysimple{v}{\tail(\evalss{w},\dur(u))} $
\evaltablend
from the evaluation \evalsym\ def:
\evaltable[]
 $\equivalence{\evalss{(u \seq v) \applyop w}}$  
 	& \ra & $\equivalence{\applysimple{seq(u, v)}{\evalss{w}}}$ with $w''= \evalss{w}$\\
	& \ra &  $\equivalence{seq(\applysimple{u}{\head(\evalss{w},\dur(u))}, \applysimple{v}{\tail(\evalss{w},\dur(u))})}$ \\
	& \ra &  $\head(seq(\top(\applysimple{u}{\head(\evalss{w},\dur(u))}, \bounds(v)),\top(\applysimple{v}{\tail(\evalss{w},\dur(u))},\bounds(v))),\bounds(l))$ \\
	& \ra &  $\head(seq(\top(h_{uw}', \bounds(v)), \top(t_{uw}', \bounds(v))),\bounds(l))$ \\
	& &  \: where $h_{uw}' = \applysimple{u}{(\head(\evalss{w},\dur(u))}$ \\
	& &  \: and   $t_{uw}' = \applysimple{v}{\tail(\evalss{w},\dur(u))}$ \\
...
\begin{center} \begin{tabular*}{0.9\linewidth}{rclcl}
	$h_{uw}'$ 	& $=$ & $\applysimple{u}{(\head(\evalss{w},\dur(u))}$ & $=$ & $h_{uw}$ \\
	$t_{uw}'$ 	& $=$ & $\applysimple{v}{\tail(\evalss{w},\dur(u))}$ 	& $=$ & $t_{uw}$ 
\evalspace
	$l_{uw}'$ 	& $=$ & $\length(h_{uw}')$	& $=$ & $\length(\applysimple{u}{(\head(\evalss{w},\dur(u))})$	\\
	$l_{uw}$ 	& $=$ & $\length(\apply{u}{\head(\evalss{w},\dur(u))})$	& $=$ & $\length(\apply{u}{\head(\evalss{w},\dur(u))})$	
\evaltablend


thus \hspace{1cm} $\evalexpr{score \applyop w}$ 	\ra $\: \equivalence{\evalss{score \applyop w}}$
