...
\sloppy
\newenvironment{gmnsyntax}		{\vspace{-2mm} {\small}}{\vspace{-1mm}}
...
Although music operations pose more complex issues at score level than with most of the composing 
...
\begin{figure}[h]
	\centering \includegraphics[width=70mm]{imgs/4voices}
...
In particular, it introduces tags parameters like $dx$ and $dy$ to indicate exact positioning of the score elements, notes and rests format specifications, etc. 
%The figure \ref{bach} illustrates the result obtained using advanced \Guido\
%\begin{figure}[htbp]
%	\begin{center}
%	\includegraphics[width=\columnwidth]{imgs/bach-inv}
%	\caption{A more complex example}
%	\label{bach}
%	\end{center}
%\end{figure}
...
The \GAR\ library is based on a single \code{guidoelement} class that contains a name and an attributes list (see figure \ref{guidoelt}). A \code{guidoattribute} has a name, a value and an optional unit (used for exact positioning).
\begin{figure}[htbp]
	\begin{center}
...
The  \code{guidoelement} class is specialized to represent the main elements of the notation (see figure \ref{classes}). Guido tags are consistently covered by the \code{guidotag} class, derived into as many types as existing tags using templates.
\begin{figure}[htbp]
	\begin{center}
...
Score level operations provided by the \GAR\ library are given by the table \ref{operations}. These operations are available as library API calls and as command line tools as well. Almost all of the operations takes a GMN score and a value parameter as input and produce a GMN score as output, but the value parameter can be indicated using another GMN score.
...
name		&	description \\
\hline
guidoseq 		& puts 2 scores in sequence \\
guidopar		& puts 2 scores in parallel \\ 
guidorpar		& puts 2 scores in parallel but right aligned \\
guidotop		& takes the top voices of a score; \\
				& when using another score as parameter, the \\
				& the voice number is taken from the score \\
guidobottom	& takes the bottom voices of a score \\
guidohead		& takes the head of a score on a time basis; \\
				& when using another score as parameter, the \\
				& cut date is taken from the score duration \\
guidoevhead	& id. but on events basis i.e. the cut \\
				& point is specified in events count; \\
				& when using another score as parameter, the \\
				& events count is taken from the score \\
guidotail		& takes the tail of a score \\
guidoevtail	& id. but on events basis \\
guidotranspose	& transposes a score to an interval $i$; \\
				& when using another score as parameter, the \\
				& interval is the difference between the \\
				& first voice, first notes of the scores \\
guidoapplypitch	& applies the pitches of a score to a score \\
guidoapplyrythm	& applies the rhythm of a score to a score \\
\end{tabular}
\end{center}
\label{operations}
\end{table}%
...
would give \hspace{3.1mm} \code{[\gtag{clef}<"g"> c d \gtag{clef}<"g"> e c ]} as result (see figure \ref{ex3}) while the clef repetition is unexpected.
\begin{figure}[h]
...
The above rules solve most of the notation consistency issues but they do not permit the operations to be reversible. Consider a score including a slur, sliced in the middle of this slur and restored by putting the parts back in sequence. The resulting score will include two slurs (see figure \ref{ex4}) instead one, due to the rules that enforce closing \oend\ tags and starting \obeg\ tags.
...
\end{itemize}
No additional provision is made for the other structure control elements: possible inconsistencies are ignored and this choice preserves the operations reversibility.
